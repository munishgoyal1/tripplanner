...
| `dev/emergency-control.ps1` | Extensible status/disable/enable orchestrator for every registered cloud emergency control; provider scripts retain identity allowlists and approval gates |
| `dev/emergency-bringdown.ps1` | Canary/production hosted-serving status and reversible bringdown; leaves provider access, data, DNS, and resources unchanged |
...
| `win/user/emergency/Emergency-Control.cmd [status\|disable\|enable] [all\|google\|azure\|local\|canary\|prod]` | Single emergency entry point for cost, legal, security, or operational shutdowns; `disable all` immediately handles every registered service and enable retains spend approvals |
| `win/user/emergency/Emergency-Bringdown.cmd [status\|down\|up] [all\|canary\|prod]` | Stop or restore all hosted Tripplanner serving independently of paid-service access; down is immediate and up requires spend approval |
