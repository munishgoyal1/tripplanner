...
def worker_prompt(
    ...
    *,
    ...
    lines += [
        ...
        "2. Make the change.",
        "3. Validate the changed ownership boundary. First run:",
        f"   `python scripts/dev/test_selection.py --base {base_sha}`",
        "   then run every command it prints from this worktree. The selector fails closed",
        "   to a complete applicable suite when a production path is not mapped. Integration",
        "   runs the complete publication suite after your branch is merged. Set",
        "   `PYTHONPATH=src` for every Python test command so the shared environment imports",
        "   this worktree rather than the primary checkout.",
