...
| WEB-01 | Responsive four-pane planning workspace | Implemented; mobile sheets provide dialog semantics, initial focus, contained keyboard navigation, Escape/backdrop dismissal, and focus restoration, while real-pane integration tests guard stale focus, authoritative mutations, and atomic trip switching across Itinerary, Map, and Details |
...
| OPS-03 | Owner-only Business, Trips, API & Cost, Infra, and System Health operations dashboard | Implemented; hidden route and server-side verified-email guard; one inclusive date filter governs durable consented visitors/page views, new trips, existing-trip updates, chat turns, and provider cost; Trips shows every persisted user/trip row with requested destination, first-build duration when recorded, iteration/chat counts, and feedback; Infra counts current Cosmos records by collection on refresh and exposes only allowlisted non-secret runtime/provider configuration; API & Cost retains its content-free environment-to-operation hierarchy, known-cost estimates, cache effectiveness, unknown-price coverage, and explicit unallocated shared-infrastructure boundary |
| OPS-04 | Repeatable Azure and Google account and billing migration | Guarded; checked-in non-secret manifests and macOS/Windows one-click launchers separately recreate all applicable Azure environment infrastructure, exactly copy hosted Cosmos data, or compose both with target validation and billing-discovered in-place Google project moves; stable target-derived run IDs resume evidence checkpoints, while DNS cutover, source retirement, and billing-account closure retain separate approval/manual gates |
...
| QUALITY-01 | Offline scenario-fidelity and experiential-quality audit | Implemented; the owner-only `/evals` console (2026-09-23) publishes a committed offline report of deterministic whole-corpus business-logic probes, place-resolution identity, audit rules and `itinerary-v1` judgements of 12 corpus trips whose original requests are reconstructed from the generation matrices, with ranked findings and efficiencies; milestone 3 adds optional advisory whole-itinerary model scores, explicit judge budgets, cached evidence-linked assessments and artifact-matched human comparison (issue #353; live quality calibration pending); milestone 2 adds per-family incremental receipts, explicit JSON trip inputs, active/historical/regression selection, preserved failure evidence and verified post-fix lifecycle (issue #351); every audit writes an immutable dated JSON report, readable summary, compact history index, comparable-run movement, scenario/preference and budget hard gates, and six non-gating experiential dimensions; stored-fact rendering is provider-denied and substitutes coordinates as well as details, photos, discovery, and browser-map configuration; the unified harness also correlates scenario/action evidence and reports measured usage, catalog-estimated cost, cache effectiveness, request amplification, performance, deterministic plan quality, model rounds, throttles, retry delay, tokens, and release Git SHA while keeping subjective evaluation optional and separately costed; a hermetic six-scenario comparison gate requires materially fewer rounds, bounded p95 latency, no quality regression, and successful degradation before behavior-sensitive runtime policy changes; fresh corpus generation explicitly identifies its paid execution scope, enforces its INR run and cumulative caps against measured model cost plus known Google catalog estimates, records that breakdown separately from provider-billed cost, and remains an explicit paid refresh; generated and persisted findings require a preventive executable fix plus a focused regression test while preserving the failing evidence, genuine fixture corrections require evidence-contract validation, and integrated fixes record deterministic post-fix replay |
...
- Google Places supplies place search, ratings, reviews, photos, restaurants,
  ...
  default to one per place, and a per-request Place Photo media ceiling of 50
  (one photo per place) bounds cold gallery fetches. Places query results, reviews, place-hours details, Routes responses,
...
- Local shared Places and global tool results use `tripplanner-cache` as an
  optional secondary durable lookup after primary misses. Hits preserve original
  observation timestamps and promote into the lane; new results write through
  with ETag-protected timestamp merges. Reads and writes fail open, and the
  feature is disabled by default in canary and production.
...
- Provider call counts and model tokens are measured. Dollar values are versioned
  ...
  catalog estimates and keep unknown-price interaction counts beside them. Provider
  billing exports remain authoritative. Shared Azure infrastructure is not fabricated
  as a per-trip amount and remains explicitly unallocated until authoritative billing
  allocation exists.
...
- All eight application containers can be exported to a portable checksummed
