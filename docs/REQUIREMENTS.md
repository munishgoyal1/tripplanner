...
| OPS-04 | Repeatable Azure and Google account and billing migration | Guarded; one manifest-driven PowerShell entry point inventories both estates, provisions a parallel Azure target with budgets, exactly transfers all eight Cosmos containers, moves Google projects and billing in place, validates targets, cuts over DNS, and retires only allowlisted source scope through independent approval gates and resumable evidence checkpoints; subscription and billing-account closure remain manual owner operations |
