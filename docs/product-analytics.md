...
1. In Google Analytics, create one GA4 property and a Web data stream for
   `https://aitripplanner.co`.
2. Disable Google Signals and ads personalization for the property unless a
   later privacy review explicitly approves them.
3. Add the stream's public `G-...` measurement ID to the ignored `.env.prod`:

   ```text
   GOOGLE_ANALYTICS_MEASUREMENT_ID=G-XXXXXXXXXX
   ```

4. Deploy through the normal canary and production promotion flow. Analytics is
   runtime-gated to `TRIPPLANNER_ENVIRONMENT=prod`; local and canary return a
   disabled configuration even if an ID is present.
5. On production, choose **Allow analytics**, then use GA4 DebugView or Realtime
   to confirm `page_view` and one normal planning flow. Reopen the choice from
   Account -> Analytics preferences.
