...
| `src/tripplanner/harness/corpus.py` `scope_places` | Gives each generated-final and emulator record only the stored place facts its plan can look up (names in its text plus derived airport, railway-station and bus-stand endpoints), so incremental receipts no longer serialise and hash the whole shared cache; audit results are unchanged |
| `src/tripplanner/tools/day_order.py` | Pure save-time day repairs called from `trip_planner._save_active_trip`: moves a stay being left to before the day's first departure and checks a selected stay out on the day the trip ends with its journey home; reports (never re-sorts) backward times. Shared with the eval probes |
| `src/tripplanner/tools/request_asks.py` | Deterministic explicit-ask detection over the traveller's words (`graph_policy.trip_request_text`): include flights, verify entry requirements, and named places or circuits must be met or listed in the trip's `dropped_requests` with a reason; added to first-turn core completion gaps. `trip_validation` adds summary-drift and implausible long-drive gaps to both gap tiers |
| `src/tripplanner/evals/probes.py`, `evals/trip_text.py` | Pure whole-corpus business-logic probes (placeholders, unpriced trips, out-of-order stops, stays after departure, summary drift, unused nights, implausible drives, sandbox fares) and place-resolution identity over the committed place cache; pointer-annotated trip text and the per-plan place subset for judge packets. Probes are rule candidates, not registered audit rules |
| `scripts/dev/owner_evals.py`, `src/tripplanner/evals/owner_report.json`, `corpus/evals/` | Offline owner evaluation report: `packets` reconstructs each corpus trip's request from the deterministic generation matrices and writes judge evidence plus readable trip text; `publish` validates `corpus/evals/judgements/*.judgement.json` with `evals.judge.validate`, runs the probes, folds in a `trip_audit.py --json` summary and the curated `corpus/evals/findings.json`, and writes the packaged report that `GET /ops/evals` serves to the owner (`frontend/src/ops/EvalsDashboard.tsx` at `/evals`). Never plans or calls a provider |
...
| `infra/migration/` | Approval-gated cloud account migration. Cross-platform one-click launchers compose target provisioning, exact Cosmos transfer, validation, and billing-discovered in-place Google moves from checked-in non-secret manifests and resumable target-derived run IDs; the phase orchestrators separately own DNS cutover and explicitly approved source retirement |
...
| `frontend/src/ops/OpsDashboard.tsx` | Direct-only Business, Trips, API & Cost, Infra, and System Health views; one inclusive date filter governs business activity and cost; Infra refreshes Cosmos collection counts and allowlisted configuration without secrets; Trips joins every saved itinerary to its user, requested destination, first-build duration, iterations, chat turns, and feedback; server authorization remains authoritative |
