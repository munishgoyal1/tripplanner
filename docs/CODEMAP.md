...
| `infra/migration/` | Approval-gated cloud account migration. The shared orchestrator owns Azure inventory, target provisioning, exact Cosmos transfer, cutover, evidence, and allowlisted source retirement; `infra/migration/google/` separately owns billing-discovered, in-place Google organization and billing moves with old-principal retirement |
