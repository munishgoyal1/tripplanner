...
| Trip cost evidence and what-if | `decisions/trip_cost.py`, `decisions/budget_what_if.py`, `providers/fx.py`, `web/budget.py`, and `web/trip_view.py` | Classify selected items as live, stale, unverified or unpriced; compare persisted provider-neutral exact products only when mandatory costs and FX are known; apply consented public benefit terms without payment data; expose expired finalized-trip quote recheck work without I/O or mutation |
| Advisory effort intelligence | `tools/trip_effort.py`, persisted weather, structured place evidence, and `web/trip_view.py` | Rank with physical, transit, logistical, circadian, and weather-exposure effort; emit at most grounded advisory notes and one pacing statement; never block or infer absent evidence |
