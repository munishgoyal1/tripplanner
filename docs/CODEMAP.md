...
| `src/tripplanner/observability.py` | Structured events and request diagnostics; full telemetry still reaches observers/ledgers/flight evidence, while console and rotating app logs show human-readable API, LLM, tool, provider, workflow, failure, and per-interaction summary lines instead of successful per-cache/per-storage noise |
