...
def _partitioned_cosmos(monkeypatch, store: dict) -> None:
    """A durable layer that, like Cosmos, finds an item only under its own partition."""
    from tripplanner import storage_cosmos
    ...
    monkeypatch.setattr(
        storage_cosmos,
        "upsert_doc",
        lambda container, partition, doc_id, body: store.__setitem__((partition, doc_id), body),
    )
    monkeypatch.setattr(
        storage_cosmos,
        "read_doc",
        lambda container, partition, doc_id: store.get((partition, doc_id)),
    )
    monkeypatch.setattr(
        storage_cosmos,
        "delete_doc",
        lambda container, partition, doc_id: store.pop((partition, doc_id), None),
    )


def test_places_are_spread_across_bucketed_partitions_not_one(_isolate, monkeypatch):
    """Every place under ``_shared`` pinned the container's load to one logical partition."""
    from tripplanner import place_cache_layout

    store: dict = {}
    _partitioned_cosmos(monkeypatch, store)
    names = [f"Place {index}" for index in range(40)]

    pc.prefetch(names, "Goa", max_photos=0, with_reviews=False)
    assert pc.flush_writes()

    partitions = {partition for partition, _item_id in store}
    assert place_cache_layout.LEGACY_PARTITION not in partitions
    assert len(partitions) > 20  # 40 hashed ids over 256 buckets
    for name in names:
        item_id = pc._doc_id(pc._key(name, "Goa"))
        assert (place_cache_layout.partition(item_id), item_id) in store


def test_legacy_shared_row_is_served_without_google_and_rehomed(_isolate, monkeypatch):
    """Rows written before bucketing carry ttl -1; dropping them would re-buy every place."""
    from tripplanner import place_cache_layout, storage_cosmos

    store: dict = {}
    _partitioned_cosmos(monkeypatch, store)
    key = pc._key("Taj", "Goa")
    item_id = pc._doc_id(key)
    entry = {
        "place_id": "legacy-taj",
        "name": "Taj",
        "lat": 15.0,
    ...
    store[(place_cache_layout.LEGACY_PARTITION, item_id)] = {"key": key, "entry": entry, "ttl": -1}

    result = pc.get_details("Taj", "Goa")

    assert result and result["place_id"] == "legacy-taj"
    assert _isolate["lookup"] == 0
    assert pc.flush_writes()
    rehomed = store[(place_cache_layout.partition(item_id), item_id)]
    assert rehomed["entry"]["place_id"] == "legacy-taj"
    assert rehomed["entry"]["__at__"] == entry["__at__"]  # observation time is not refreshed

    # A fresh process now finds the bucketed copy and never reads the legacy partition.
    legacy_reads: list[str] = []
    bucketed_read = storage_cosmos.read_doc

    def spying_read(container, partition, doc_id):
        if partition == place_cache_layout.LEGACY_PARTITION:
            legacy_reads.append(doc_id)
        return bucketed_read(container, partition, doc_id)

    monkeypatch.setattr(storage_cosmos, "read_doc", spying_read)
    pc.clear_cache()
    assert pc.get_details("Taj", "Goa")["place_id"] == "legacy-taj"
    assert legacy_reads == []


def test_stale_legacy_row_is_rehomed_even_when_read_only(_isolate, monkeypatch):
    """A read-only projection may serve a lapsed row that never enters L1; it still moves."""
    from tripplanner import place_cache_layout
    ...
    key = pc._key("Taj", "Goa")
    item_id = pc._doc_id(key)
    stale = {"place_id": "old", "name": "Taj", "lat": 1.0, "lng": 2.0, "__at__": 1.0}
    store[(place_cache_layout.LEGACY_PARTITION, item_id)] = {"key": key, "entry": stale}

    with _read_only():
        assert pc.get_details("Taj", "Goa")["place_id"] == "old"

    assert pc.flush_writes()
    assert store[(place_cache_layout.partition(item_id), item_id)]["entry"]["place_id"] == "old"
