...
const contrasts: Record<string, ContrastDefinition> = {
  "honest-gaps": {
    axis: "Every option finds the same eight open decisions in the same Goa trip and resolves each with the same suggestions. They disagree about where those gaps are gathered and how visible they stay while the traveller reads the plan.",
    rows: [
      { option: "A · Decision inbox", score: 88, idea: "A pinned bar above the days counts the open decisions; it expands into one list where each gap has three suggestions and a Choose myself action. Placeholder rows are drawn dashed.", buys: "The whole gap set can be seen and cleared in one place without scrolling four days, and the count shrinks visibly as the plan becomes real.", costs: "One more persistent strip at the top of the itinerary, and a list view that is separate from the day where the gap sits.", choose: "The first plan usually arrives with several gaps and the traveller wants to close them in one sitting." },
      { option: "B · Inline slots", score: 81, idea: "Each placeholder becomes a dashed slot card in its own day with suggestion chips inside it. Only day headers carry a small count.", buys: "The gap is resolved exactly where it matters in time and place, next to the stops before and after it.", costs: "Seeing the full set means reading every day, and four slots on a busy day make the pane longer.", choose: "Gaps are few and the traveller reads the plan day by day anyway." },
      { option: "C · Readiness checklist", score: 67, idea: "Details gains a Plan readiness checklist grouped by Stay, Meals, Journeys and Prices. Itinerary rows only gain a small hollow marker.", buys: "The calmest itinerary and a clear per-category progress story beside the existing booking meter.", costs: "The gap lives in a different pane from the stop it describes, so the placeholder still mostly looks like a place.", choose: "The itinerary must stay visually unchanged and Details is already open." },
      { option: "D · Assistant follow-up", score: 52, idea: "After delivering the plan the Assistant posts one message listing the gaps as tappable chips. The panes are unchanged.", buys: "No new UI surface at all, and the ask reads naturally as a conversation.", costs: "The list scrolls away with the transcript and the itinerary keeps presenting TBD stops as if they were decided.", choose: "Only when no pane may change for now." },
    ],
    same: "The same Goa fixture, the same eight gaps (the hotel, five TBD meals, an untimed outbound flight and a sandbox return fare), the same three suggestions per gap and the same Choose myself path. In every option a gap is never pinned on the map as if it were a real place, resolving a gap updates the itinerary row, and nothing is booked or re-priced.",
    verdict: "The decision inbox ranks first because the offline evaluation found gaps in all twelve judged corpus trips, usually several per trip, so the common job is to clear a set of them, and only a single list does that without a scroll through every day. Inline slots are the strongest companion rather than a rival: once a traveller opens a day, the slot is the most natural place to fix it, and the best build would likely pair A's count with B's slots. The readiness checklist is calm but leaves the itinerary row looking decided, which is the defect this Lab exists to remove. The Assistant follow-up changes nothing the traveller reads, so it ranks last.",
  },
  ...
  "planner-layout-directions": {
    ...
    rows: [
      { option: "A · Journey canvas", score: 95, idea: "A map-led stage with a horizontal day journey, a focused place inspector, and an assistant command dock.", buys: "The trip reads as a route at first glance, while detail appears only for the stop currently in focus.", costs: "The largest implementation change and the biggest departure for people already fluent in the three-pane workspace.", choose: "Spatial orientation and direct manipulation should define the planner." },
      { option: "B · Trip storyboard", score: 91, idea: "An editorial day story occupies the main canvas, supported by a compact chapter rail and a live map/evidence column.", buys: "The most readable planning experience, with strong scan order and a clear narrative from morning to evening.", costs: "The map becomes supporting evidence rather than the constant center of gravity.", choose: "Reading, comparing, and refining the itinerary should feel calmer than operating a dashboard." },
      { option: "C · Refined spatial workspace", score: 88, idea: "Keep sbx4's itinerary-map-details composition while rebuilding its hierarchy, grouping, density, colour, and assistant dock.", buys: "A marked quality lift with familiar navigation and a credible migration path from the current implementation.", costs: "It inherits the three-pane model's competition for width on smaller laptops.", choose: "The product needs a substantial upgrade without retraining existing users." },
      { option: "D · Precision polish", score: 80, idea: "Keep sbx4's exact geometry and information order, refining icons, pane controls, spacing, states, and visual finish.", buys: "The lowest-risk route: professional hide/maximize affordances and cleaner control groups with little structural work.", costs: "The underlying density and split-pane compromises remain unchanged.", choose: "Continuity and implementation speed matter more than a new workspace model." },
    ],
    same: "The Kyoto trip title and dates, four days, ten stops, 60% readiness, selected day and place, warning content, grouped trip actions, compact feedback control, settings and sign-in, assistant suggestions and composer, and all underlying planner behaviour. Each preview uses the same 1440-by-900-class application frame and the same warm professional colour family.",
    verdict: "Journey canvas ranks first because it makes geography and sequence feel like one object rather than adjacent panes, which is the planner's strongest distinctive opportunity. Trip storyboard is nearly as strong and is the easiest to read, but gives up some persistent spatial context. Refined spatial workspace is the pragmatic implementation candidate because it preserves learned navigation while fixing the visible quality gaps. Precision polish is deliberately fourth: it is valuable and safe, but it cannot resolve the space competition created by the existing structure.",
  },
  "trip-feedback": {
    axis: "Every option collects the same rating, the same thumbs pair, and the same optional comment, and every one of them accepts a single tap as a finished submission. They disagree about where the control rests, how much of the workspace it holds before it is used, and whether feedback is always reachable or only offered at one moment.",
    rows: [
      { option: "A · Toolbar rating pill", score: 93, idea: "A small thumbs pair lives in the workspace toolbar and opens a compact rating popover.", buys: "Always one tap away from every pane, at almost no resting cost to the plan.", costs: "The popover must stay small, so a long comment gets a modest box rather than a page.", choose: "Feedback should be possible at any moment rather than at one scripted point." },
      { option: "B · Itinerary footer card", score: 87, idea: "A calm card sits after the last day, at the natural end of reading the plan.", buys: "It arrives exactly when the traveller has just finished forming an opinion.", costs: "Only reachable at the bottom of the itinerary, so it is not truly always available.", choose: "The honest moment to ask is after the whole plan has been read." },
      { option: "C · Assistant-led ask", score: 85, idea: "The planner asks once in the conversation after it delivers the itinerary.", buys: "The lowest-friction answer of all, because it reads as replying rather than rating.", costs: "The ask scrolls away with the transcript and can feel like nagging if repeated.", choose: "The conversation is already where the traveller reacts to the plan." },
      { option: "D · Per-day thumbs", score: 81, idea: "Each day header carries its own thumbs pair, rolling up into one trip rating.", buys: "The most useful signal by far: it says which day was wrong, not just that something was.", costs: "Adds a control to every day row and makes a single trip-level verdict ambiguous.", choose: "Knowing which part of the plan failed matters more than a single score." },
      { option: "E · Floating feedback tab", score: 76, idea: "A discreet anchored pill floats over the workspace and opens a sheet.", buys: "Permanently visible and completely independent of the itinerary layout.", costs: "It reads as a third-party survey widget and always covers a corner of the plan.", choose: "Feedback must be reachable even on surfaces that have no toolbar." },
    ],
    same: "The same five-star rating, the same thumbs up/down pair, and the same optional comment, in that same optional order. A single thumb tap is a complete submission in all five. Every option records the submission against the trip, attributes it to the signed-in user when there is one, allows unlimited later submissions, shows the same quiet already-sent marker with a count, never blocks the workspace, and never requires a rating to leave a comment or a comment to leave a rating.",
    verdict: "A wins because the requirement is that feedback is always possible, and the toolbar is the only place that is equally close to the itinerary, the map, the details, and the assistant while costing almost no resting space. B is the strongest companion rather than a rival: the end of the itinerary is the most honest moment to ask, and it pairs naturally with a persistent control. C produces the richest comments because replying is easier than rating, but a transcript is the wrong place to keep a permanently available action. D collects the most actionable signal and is the best future addition once a trip-level verdict already exists. E is last because it buys permanent visibility at the cost of looking like bolted-on survey furniture over a product whose whole value is the plan underneath it.",
  },
  "profile-workspace": {
