  ...
  return (
    <main className="min-h-full bg-[linear-gradient(180deg,#f8fafc_0,#ecfdf5_32rem,#f8fafc_100%)] px-4 py-7 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[92rem]">
        <LabNavigation detail labId="map-controls" />
        <header className="mt-4 flex flex-wrap items-end justify-between gap-5 border-b border-slate-200 pb-5"><div><div className="flex items-center gap-2 text-accent"><Map size={17} /><p className="text-[10px] font-bold uppercase">Active UX Lab</p></div><h1 className="display mt-2 text-3xl font-semibold text-ink">Map commands and day context</h1><p className="mt-2 max-w-4xl text-sm leading-relaxed text-slate-600">Compare three complete control systems for the Map’s most frequent work: switch between all circuits and one day, understand full schedule versus route-only travel, add a stop to an explicit day, and inspect an exact place without losing context.</p></div></header>
        <LabScope labId="map-controls" />
        <OptionContrast labId="map-controls" />
        ...
        <section data-lab-change="Map controls and day context" className="mt-2 h-[720px] max-h-[78vh] min-h-[620px] overflow-hidden rounded-md bg-white shadow-pop ring-1 ring-slate-200" aria-label="Interactive Map controls preview"><MapWorkspace key={variant} variant={variant} /></section>
