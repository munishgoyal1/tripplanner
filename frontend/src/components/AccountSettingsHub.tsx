...
function SectionRail({ active, onSelect }: { active: Section; onSelect: (id: Section) => void }) {
  return (
    <nav aria-label="Profile sections" className="flex shrink-0 gap-1 overflow-x-auto border-b border-border bg-background/60 p-2 lg:w-64 lg:flex-col lg:gap-0.5 lg:overflow-visible lg:border-b-0 lg:border-r lg:p-3">
      {SECTIONS.map(({ id, label, detail, icon: Icon }) => (
        <button
          ...
          className={`flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-left transition ${
            active === id ? "bg-sand text-ink ring-1 ring-border" : "hover:bg-paper/70"
          }`}
        >
          <Icon size={16} className={active === id ? "shrink-0 text-brand" : "shrink-0 text-muted"} aria-hidden />
          <span className="min-w-0">
            <span className="block whitespace-nowrap text-xs font-semibold text-ink">{label}</span>
            <span className="mt-0.5 hidden truncate text-[11px] text-muted lg:block">{detail}</span>
          </span>
        </button>
      ))}
    </nav>
...
function SectionHeader({ icon: Icon, eyebrow, title, description }: {
  ...
  return (
    <div className="mb-5 flex items-start gap-3">
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-md bg-clay-soft/60 text-clay"><Icon size={17} aria-hidden /></span>
      <div>
        <p className="text-[10px] font-bold uppercase text-brand">{eyebrow}</p>
        <h3 className="display mt-0.5 text-2xl text-ink">{title}</h3>
...
export default function AccountSettingsHub({
  ...
  return (
    <aside aria-label="Account settings" className="fixed inset-0 z-[100] flex flex-col bg-paper">
      <header className="flex h-14 shrink-0 items-center border-b border-border px-4 lg:px-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0"><h2 className="truncate text-sm font-semibold text-ink">{auth.authenticated ? auth.display_name || "Signed in" : localIdentityActive ? nameInput || "Local traveler" : "Guest traveler"}</h2><p className="truncate text-[11px] text-muted">{auth.email || (localIdentityActive ? "Local identity" : "Sign in to sync across devices")}</p></div>
        </div>
        <button type="button" onClick={onClose} className="ml-auto grid h-8 w-8 shrink-0 place-items-center rounded-md text-muted hover:bg-sand hover:text-ink" aria-label="Close account settings"><X size={16} aria-hidden /></button>
      </header>

      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <SectionRail active={destination} onSelect={setDestination} />

        <div className="min-h-0 flex-1 overflow-y-auto bg-sand/60 p-4 lg:p-8">
          <div className="mx-auto max-w-4xl rounded-lg border border-border bg-paper p-4 shadow-card sm:p-6">
            {destination === "profile" && <>
              ...
              {auth.authenticated ? <>
                <div className="flex items-center gap-3 rounded-md bg-background p-3 ring-1 ring-border">
                  ...
                  <div className="min-w-0"><p className="truncate text-sm font-semibold text-ink">{auth.display_name || "Signed in"}</p>{auth.email && <p className="truncate text-xs text-muted">{auth.email}</p>}<p className="mt-1 inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700"><Check size={11} aria-hidden /> Google account connected</p></div>
                </div>
                <button type="button" onClick={onSignOut} disabled={privacyBusy} className="mt-4 inline-flex h-9 items-center gap-2 rounded-md px-3 text-xs font-semibold text-muted ring-1 ring-border hover:bg-background disabled:opacity-50"><LogOut size={14} aria-hidden /> Sign out</button>
              </> : <div className="space-y-3">
                {googleEnabled && <button type="button" onClick={onGoogleSignIn} className="flex h-10 w-full items-center justify-center rounded-md bg-paper text-xs font-semibold text-ink ring-1 ring-border hover:bg-background">Sign in with Google</button>}
                <div><label htmlFor="account-display-name" className="text-xs font-semibold text-muted">Display name</label><input id="account-display-name" value={nameInput} onChange={(event) => onNameInputChange(event.target.value)} onKeyDown={(event) => event.key === "Enter" && nameInput.trim() && onLocalSignIn()} className="mt-1 h-9 w-full rounded-md border border-border px-3 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" placeholder="Your name" /></div>
                <div className="flex items-center gap-2"><button type="button" onClick={onLocalSignIn} disabled={!nameInput.trim()} className="h-9 rounded-md bg-brand px-4 text-xs font-semibold text-white disabled:opacity-40">{localIdentityActive ? "Update name" : "Sign in"}</button>{localIdentityActive && <button type="button" onClick={onSignOut} className="h-9 rounded-md px-3 text-xs font-semibold text-muted hover:bg-background">Sign out</button>}</div>
              </div>}
            ...
            {destination === "privacy" && <>
              ...
              <div className="divide-y divide-border/60 rounded-md ring-1 ring-border">
                <button type="button" onClick={onDeleteTripHistory} disabled={privacyBusy} className="w-full p-3 text-left hover:bg-background disabled:opacity-50"><span className="block text-xs font-semibold text-ink">Delete trip and chat history</span><span className="mt-1 block text-[11px] leading-relaxed text-muted">Remove saved trips and related conversations while retaining your account and travel profile.</span></button>
