...
export default function TripFeedbackControl({ disabled = false, initial, mobile = false }: Props) {
  ...
  return (
    <div ref={rootRef} role="group" aria-label="Rate this trip" className={`relative flex shrink-0 items-center ${mobile ? "gap-0" : "gap-0 rounded-full border border-border bg-sand p-0.5"}`}>
      ...
      {open && (
        <section className={`z-[90] border border-border bg-paper p-3 shadow-pop ${mobile ? "fixed inset-x-3 bottom-16 rounded-lg" : "absolute right-0 top-full mt-2 w-80 rounded-lg"}`} aria-label="Trip feedback">
          <div className="flex items-start justify-between gap-3">
            <div><h3 className="text-sm font-semibold text-ink">How does this trip read?</h3><p className="mt-0.5 text-xs text-muted">One tap is enough. Stars and a note are optional.</p></div>
            <button type="button" onClick={() => setOpen(false)} className="grid h-7 w-7 place-items-center rounded-md text-muted hover:bg-sand" aria-label="Close feedback"><X size={14} /></button>
          </div>
          ...
          <textarea value={comment} onChange={(event) => setComment(event.target.value)} maxLength={1000} placeholder="Optional: what would you change?" className="mt-3 min-h-20 w-full resize-none rounded-md border border-border px-3 py-2 text-sm outline-none focus:border-brand" />
